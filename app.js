...
function unitPrice(p){
  return (p.offerPrice && Number(p.offerPrice) < Number(p.price)) ? Number(p.offerPrice) : Number(p.price);
}
...
function render(){
  ...
  $("#brandTagline").textContent=data.tagline;
  $("#footerBrand").textContent=data.brand;
  $("#footerTagline").textContent=data.tagline;
...
function renderCategories(){
  $("#categoryTabs").innerHTML=data.categories.map(c=>
    `<button type="button" class="${c===activeCategory?'active':''}" data-cat="${c}">${c}</button>`
  ).join("");
  ...
  $("#foodGrid").innerHTML=items.map(p=>{
    ...
    return `<article class="food-card">
      <div class="food-media">
        ...
        ${p.badge?`<span class="food-badge">${p.badge}</span>`:""}
      </div>
      <div class="food-body">
        <h3>${p.name}</h3>
        <p>${p.description||""}</p>
        <div class="food-bottom">
          ...
          <button class="add-btn" ${unavailable?"disabled":""} onclick="addToCart(${p.id})">${unavailable?"বন্ধ":"কার্টে যোগ"}</button>
        </div>
      </div>
    </article>`;
  }).join("");
  ...
  if(!cart.length){
    ...
    $("#cartItems").innerHTML=cart.map(c=>{
      ...
      return `<div class="cart-row">
        ...
        <div><strong>${x.name}</strong><div class="qty"><button type="button" onclick="changeQty(${x.id},-1)">−</button><span>${c.qty}</span><button type="button" onclick="changeQty(${x.id},1)">+</button></div></div>
...
function changeQty(id,d){
  const x=cart.find(c=>c.id===id); if(!x)return;
  x.qty+=d;
...
function showWaFallback(waLink,webLink,msg){
  ...
  if(!box){
    box=document.createElement('div'); box.id='waFallback'; box.className='checkout-modal'; box.setAttribute('aria-hidden','true');
    box.innerHTML=`<div class="checkout-backdrop" onclick="document.getElementById('waFallback').classList.remove('open')"></div><div class="checkout-card"><div class="checkout-head"><div><span class="section-kicker">WHATSAPP</span><h3>অর্ডার পাঠান</h3></div><button type="button" onclick="document.getElementById('waFallback').classList.remove('open')">×</button></div><div style="padding:22px"><p style="margin-top:0;color:#796d65">WhatsApp খুলতে সমস্যা হলে নিচের অপশনটি ব্যবহার করুন।</p><a href="${waLink}" target="_blank" rel="noopener" class="primary-btn full-btn" style="display:block;text-align:center;margin-bottom:10px">📱 WhatsApp App / Link</a><a href="${webLink}" target="_blank" rel="noopener" class="primary-btn full-btn" style="display:block;text-align:center;background:#20150f">🌐 WhatsApp Web</a><button type="button" class="primary-btn full-btn" style="margin-top:10px;background:#eadfd2;color:#20150f" onclick="navigator.clipboard?.writeText(${JSON.stringify(msg)}).then(()=>alert('অর্ডার মেসেজ কপি হয়েছে।'))">📋 মেসেজ কপি করুন</button></div></div>`;
...
function submitOrder(e){
  ...
  setTimeout(()=>{
    if(document.visibilityState!=='hidden')showWaFallback(waLink,webLink,msg);
  },1000);
}
